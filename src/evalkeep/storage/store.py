...
from evalkeep.redaction import RedactionSummary
from evalkeep.storage.clusters import ClusterStore
from evalkeep.storage.failures import FailureStore
...
@dataclass(frozen=True)
class TraceSummary:
    """One row of ``evalkeep trace list``."""
...
class TraceStore:
    ...
    def tests(self) -> RegressionStore:
        """Regression-test drafts, sharing this store's connection."""
        return RegressionStore(self._connection)
