...
the JSON schema in :mod:`evalsmith.prompts` rather than asked politely for JSON
...
The ``anthropic`` package is an optional dependency: Evalsmith runs entirely
...
def _build_client() -> Any:
    try:
        ...
        raise AnalyzerError(
            ...
            "Install it with: uv add 'evalsmith[anthropic]'"
        ) from exc
    try:
