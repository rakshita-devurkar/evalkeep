"""``evalsmith detect`` and ``evalsmith failures`` -- evidence and review."""
...
from evalsmith.failures import Failure, FailureStatus, failure_id_for
from evalsmith.storage import FailureSummary, StoredTrace, TraceStore
    ...
    with TraceStore.open(project.database_path) as store:
        if store.count() == 0:
            raise CommandError(
                ...
                hint="Run 'evalsmith ingest traces.jsonl' first.",
            )
    ...
    with TraceStore.open(project.database_path) as store:
        if store.get(trace_id) is None:
            raise CommandError(
                ...
                hint="Run 'evalsmith trace list' to see what has been ingested.",
            )
        ...
        if existing is not None:
            raise CommandError(
                ...
                hint=f"Use 'evalsmith failures confirm {existing.failure_id}' instead.",
            )
...
def resolve_failure(store: TraceStore, identifier: str) -> Failure:
    ...
    if failure is None:
        raise CommandError(
            ...
            hint="Run 'evalsmith failures list', or 'evalsmith detect' if you have "
            "not detected yet.",
        )
    return failure
