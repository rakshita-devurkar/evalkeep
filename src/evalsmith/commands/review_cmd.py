"""``evalsmith review`` and the non-interactive decision commands.
...
from evalsmith.commands.dataset_cmd import show_test
...
from evalsmith.review import ReviewError, apply_edits, approve, reject, render_editable
from evalsmith.storage import StoredTrace, TraceStore
    ...
    *,
    ...
    with TraceStore.open(project.database_path) as store:
        ...
        try:
            ...
            raise CommandError(
                str(exc),
                hint="Fix it with 'evalsmith dataset edit <id>' and approve again.",
            ) from exc
...
def _resolve(store: TraceStore, identifier: str) -> RegressionTest:
    ...
    if test is None:
        raise CommandError(
            ...
            hint="Run 'evalsmith dataset list' to see what exists.",
        )
    return test
