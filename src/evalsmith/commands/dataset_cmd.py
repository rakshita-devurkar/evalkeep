"""``evalsmith dataset`` -- generate and inspect regression-test drafts.
...
from evalsmith.clusters import Cluster
...
from evalsmith.generation import GENERATOR_VERSION, build_test
    ...
    with TraceStore.open(project.database_path) as store:
        ...
        if representatives_only and not clusters:
            raise CommandError(
                ...
                hint="Run 'evalsmith discover' first, or pass --all to cover every failure.",
            )
        ...
        if not targets:
            raise CommandError(
                ...
                hint="Run 'evalsmith detect' and 'evalsmith analyze' first.",
            )
    ...
    with TraceStore.open(project.database_path) as store:
        ...
        if test is None:
            raise CommandError(
                ...
                hint="Run 'evalsmith dataset list', or 'evalsmith dataset build' first.",
            )
        return test
