"""``evalsmith discover`` and ``evalsmith clusters`` -- group failures and edit groups.
...
from evalsmith.cache import EmbeddingCache
from evalsmith.clustering import ClusterInput, assign_roles, derive_label
from evalsmith.clusters import Cluster, ClusterMember
from evalsmith.commands.analyze_cmd import run_analysis
from evalsmith.commands.detect_cmd import default_reviewer
from evalsmith.config import Project
from evalsmith.discovery import ClusterEditsWouldBeLost, DiscoveryReport, discover
from evalsmith.embeddings import get_embedder
    ...
    *,
    ...
    with TraceStore.open(project.database_path) as store:
        if store.failures.count() == 0:
            raise CommandError(
                "No failure candidates to group.", hint="Run 'evalsmith detect' first."
            )
        try:
        ...
        if report.clusters == 0:
            raise CommandError(
                ...
                hint="Run 'evalsmith analyze', or label failures by hand with "
                "'evalsmith failures label'.",
            )
        return report
    ...
    with TraceStore.open(project.database_path) as store:
        ...
        if missing:
            raise CommandError(
                ...
                hint="Run 'evalsmith clusters show <id>' to see its members.",
            )
        if not staying:
            raise CommandError(
...
def _resolve(store: TraceStore, identifier: str) -> Cluster:
    ...
    raise CommandError(
        ...
        hint="Run 'evalsmith clusters list', or 'evalsmith discover' first.",
    )
